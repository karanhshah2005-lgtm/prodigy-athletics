...
import {
  renderGarment, slotsFor, STYLES, BASE_PRESETS,
...
const state = {
  ...
  activeSlot: 'all',
  activePanel: 'style',    // style | art | colour | scenes | export
  sheetOpen: false,        // mobile bottom-sheet visibility
  scenesSelected: { front: true, back: true, cutsheet: false, set: false },
...
function garmentSvgFor(style, uid, cellSize, artBucket) {
  ...
  return renderGarment({ style, view: 'front', baseColor: state.baseColor, slots, uid, size: cellSize, detail: 'lite', defs });
}
...
function scheduleGridSetThumb(key) {
  ...
  gridSetDebounce = setTimeout(async () => {
    try {
      const blob = await buildSetBlob(90);
...
// therefore appears in the 360 exactly where it appears flat.
...
function spinDesignKey() {
  ...
  return JSON.stringify([state.style, state.baseColor, art]);
}
...
async function buildSpinBake() {
  ...
  for (const view of ['front', 'back']) {
    ...
    for (const part of ['torso', 'sleeveL', 'sleeveR']) {
      ...
      const svg = renderGarment({ ...common, baseColor: state.baseColor });
...
function spinOptsFromState() {
  return {
    ...
    sleeveColor: null,
    sleeveTextColor: toneLight(state.baseColor) ? '#0B1220' : '#F5F3EE',
    chestMarkColor: toneLight(state.baseColor) ? '#0B1220' : '#F5F3EE',
...
function updateCropPreview() {
  ...
  const svg = renderGarment({ style: state.style, view: state.view, baseColor: state.baseColor, slots, uid, size: 260, detail: 'lite', defs });
