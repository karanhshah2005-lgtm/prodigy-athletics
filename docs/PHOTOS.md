...
## Concept wall (2026-09-04) — thirty anime-style concept designs, concepts.html

Owner direction: "make some more. 30 different ones, i don't see any anime ones." Thirty
AI-generated concept designs (`tools/placeholders/gen_concepts.py`, one prompt per tile,
gemini-3.1-flash-image image-edit over the logo'd plain flat-lays) at
`assets/photos/concepts/cNN-<slug>.webp` (1024², q82). Each is an ORIGINAL anime-style
character or scene — shonen power-up, samurai, mecha, kitsune, neon alley, chibi
grapplers, Raijin, phoenix… — chosen so the page reads unmistakably "anime" without
reproducing any existing anime/manga/game character (the site is public; the client
will show it around). The logo on every tile is Prodigy's, from the reference crop.
The page and every alt say "concept, not a product". Tiles + captions are generated by
the script that also wrote concepts.html (see the page's source comment); to add or
re-roll a tile, edit CONCEPTS in gen_concepts.py and re-run both.
