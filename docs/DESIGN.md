...
2. **Do not use the SAU crest, name or Invincible Fighting Championships** in any asset
   ...
   author. Exception on record: on 2026-08-26 the owner directed third-party event
   frames onto the landing panels (docs/PHOTOS.md batch 2); since 2026-08-29 (cage-banner
   frame replaced, owner direction) the promotion's floor lettering remains legible in
   landing panel 1 only. The written SAU permission that covers this is an open
   must-supply item below, and the exception extends to no other asset. The SAU crest
   itself ships nowhere.
